...
const VideoOptions = () => {
  ...
  const getSong = useCallback(async (args: { [key: string]: string }) => {
  ...
  const getRandomSong = useCallback(
    async (args: { [key: string]: string }) => {
      ...
      if (songData.none) getRandomSong(args);
        ...
        setCurrentSongData(songData.song);
      } else {
  ...
  const onChangeSeletecedGenres = useCallback(() => {
    ...
    // select random song
    if (selectedGenre) {
      getRandomSong({ genre: selectedGenre });
    }
  }, [genreState, selectedGenre, getRandomSong]);
